...
function applyWorkflowToRootTask(rootTask, workflow, ref, options = {}) {
  ...
  if (options.createAllSteps) {
    const baseDate = rootTask.workDueDate || rootTask.dueDate || toDateString(getToday());
    ...
    followUps = stepDefs
      ...
      .map((def) => {
        ...
        return buildWorkflowGeneratedTaskPayload({
...
function persistPendingTaskDraft(draft, options = {}) {
  ...
  if (options.applyWorkflow && followUps.length) {
    showToast(`${followUps.length}개의 후속 Task가 생성되었습니다.${getWorkflowTaskCreationToastSuffix()}`);
  }
...
function createWorkflowStepTask({ workflow, root, instance, stepEntry, anchorTask }) {
  ...
  const newTask = buildWorkflowGeneratedTaskPayload({
    taskName,
    rootTask: root,
    stepConfig,
...
function finalizeWorkflowTaskCompletion(completedTask, result) {
  ...
  if (result.created) {
    showToast(
      `${result.workflowName} Workflow 진행 중 — 다음 업무 "${result.nextTaskName}"이(가) 생성되었습니다.${getWorkflowTaskCreationToastSuffix()}`
    );
    return;
  }
...
function formatWorkflowStepDueHint(def, matchedTask) {
  if (matchedTask?.workDueDate) {
    return ` (업무 ${formatDueDaySuffix(matchedTask.workDueDate)})`;
  }
  if (matchedTask?.dueDate) {
    return ` (${formatDueDaySuffix(matchedTask.dueDate)})`;
  }
...
function calculateWorkflowStepWorkDueDate(baseDate, stepConfig, stepKind = "post") {
  const offset = Number(stepConfig?.dueOffset) || 0;
  const dueUnit = stepConfig?.dueUnit || "calendar";
  if (stepKind === "pre") {
    return addDaysToDate(baseDate, -Math.abs(offset));
  }
  return calculateDueDateFromRule(baseDate, offset, dueUnit);
}

function buildWorkflowGeneratedTaskPayload({
  taskName,
  rootTask,
  stepConfig,
  stepKind,
  ...
  const workDueDate = calculateWorkflowStepWorkDueDate(baseDate, stepConfig, stepKind);
  return {
    id: generateId(),
    ...
    priority: stepConfig?.priority || rootTask.priority || "Medium",
    parentTaskId: parentTaskId || rootTask.id,
    autoGenerated: true,
    workflowInstanceId: instance.id,
...
function generateWorkflowFollowUps(parentTask, rule, completedDate) {
  ...
  return taskRules
    ...
    .map((taskRule) => ({
      ...
      dueDate: "",
      workDueDate: calculateDueDateFromRule(completionDate, taskRule.dueOffset, taskRule.dueUnit),
...
function recalculateAutoSubtaskDueDates(parentId, newBaseDate, parentTaskName = "") {
  ...
  getSubtasks(parentId).forEach((subtask) => {
    ...
    const nextWorkDue = calculateDueDateFromRule(baseDate, ruleConfig.dueOffset, ruleConfig.dueUnit);
    const updates = isWorkflowConnectedTask(subtask) || subtask.workDueDate
      ? { workDueDate: nextWorkDue }
      : { dueDate: nextWorkDue };
...
function populateTaskDetailForm(task) {
  ...
  let todoWorkflowHint = document.getElementById("editTodoDueWorkflowHint");
  if (!todoWorkflowHint) {
    const dueField = document.getElementById("editDueDate");
    if (dueField?.parentElement) {
      todoWorkflowHint = document.createElement("p");
      todoWorkflowHint.id = "editTodoDueWorkflowHint";
      todoWorkflowHint.className = "form-hint form-hint--workflow-todo";
      dueField.parentElement.appendChild(todoWorkflowHint);
    }
  }
  if (todoWorkflowHint) {
...
function renderDashboardTaskWorkflowBar(task) {
  ...
  return `
    <div class="dash-task-wf-bar" aria-label="Workflow 진행률 ${pct}%">
      <div class="dash-task-wf-bar__track" role="progressbar" aria-valuenow="${completed}" aria-valuemin="0" aria-valuemax="${total}">
        <span class="dash-task-wf-bar__fill" style="width:${pct}%"></span>
      </div>
      <span class="dash-task-wf-bar__pct">${pct}%</span>
    </div>
  `;
}
  ...
  if (layout === "updates") {
    ...
    itemsHtml = grouped
      .map(([study, studyTasks]) => {
        ...
        if (!showStudyHeaders) return tasksHtml;

        return `
          <section class="dash-study-group">
            <h4 class="dash-study-group__title">${escapeHtml(study)}</h4>
            <div class="dash-study-group__list">${tasksHtml}</div>
          </section>
        `;
      })
      .join("");
